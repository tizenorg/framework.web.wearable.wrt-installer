...
const std::string WRT_CORE_SPEC_2_3_1_FEATURE = "2.3.1";
}
...
static bool parseVersionString(const std::string &version, long &majorVer, long &minorVer, long &microVer)
{
    std::istringstream inputString(version);
    inputString >> majorVer;
    if (inputString.bad() || inputString.fail()) {
        _E("Invalid minVersion format.");
        return false;
    }
    inputString.get(); // skip period
    inputString >> minorVer;
    if (inputString.bad() || inputString.fail()) {
        _E("Invalid minVersion format");
        return false;
    }
    inputString.get(); // skip period
    inputString >> microVer;
    if (inputString.bad() || inputString.fail()) {
        _E("Invalid microVersion format");
       microVer = 0;
    }
...
/*
 * Compare version to check whether the feature is allowed on this App version or not
 *
 * @ver1 : required version of App
 * @ver2 : required version of feature
 * @ret  : true when acceptable
 *
 */
static bool isAcceptableVersion(const std::string &ver1, const std::string &ver2)
{

    //Parse required version
    long major1 = 0, minor1 = 0, micro1 = 0;
    //Parse platform version
    long major2 = 0, minor2 = 0, micro2 = 0;
    if (!parseVersionString(ver1, major1, minor1, micro1)) {
        _E("Invalid format of widget version string.");
        return false;
    }

    if (!parseVersionString(ver2, major2, minor2, micro2)) {
        _E("Invalid format of platform version string.");
        return false;
    }
    ...
    if (major1 > major2 ||
       ...
       (major1 == major2 && minor1 == minor2 && micro1 > micro2)) {
        return true; // required_version > platform_version
    } else if (major1 == major2 && minor1 == minor2 && micro1 == micro2) {
        return true; // required_version == platform_version
    } else {
        return false; // required_version < platform_version
    }
}
...
class AccessParser : public ElementParser
{
  public:
...
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
            NormalizeString(normalizedValue, MAX_ATTR_ELEMENT_LENGTH, true);
#else
    ...
    {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_strIRIOrigin, MAX_ATTR_ELEMENT_LENGTH, true);
#else
  ...
  public:
    ...
    virtual void Accept(const XmlAttribute& attribute)
    {
        ...
        if (attribute.name == L"src") {
            m_src = value;
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
            NormalizeString(m_src, MAX_ATTR_ELEMENT_LENGTH, true);
#else
    ...
    virtual void Verify()
    {
        ...
        if (!!m_src) {
            ...
            if (!!m_type) {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
                NormalizeString(m_type, MAX_ATTR_ELEMENT_LENGTH, true);
#else
            ...
            if (!!m_encoding) {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
                NormalizeString(m_encoding, MAX_ATTR_ELEMENT_LENGTH, true);
#else
...
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_name, MAX_NAME_KEY_LENGTH, true);
        NormalizeString(m_value, MAX_NAME_KEY_VALUE_LENGTH, true);
#else
        NormalizeString(m_name);
...
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
            NormalizeString(m_value, MAX_ATTR_ELEMENT_LENGTH, true);
#else
            NormalizeString(m_value);
#endif
  ...
  public:
    ...
    virtual void Accept(const XmlAttribute& attribute)
    {
        if (m_properNamespace) {
            ...
            } else if (attribute.name == L"ambient_support") {
                m_ambient = attribute.value;
            } else {
                ThrowMsg(Exception::ParseError,
                         "unknown attribute '" +
    ...
    virtual void Verify()
    {
        ...
        DPL::OptionalString minVersion = m_data.tizenMinVersionRequired;
        if (!minVersion) {
            ThrowMsg(Exception::ParseError, "required_version is mandatory");
        }

        if (!!m_ambient) {
            if (!isAcceptableVersion(DPL::ToUTF8String(*minVersion), WRT_CORE_SPEC_2_3_1_FEATURE)) {
                ThrowMsg(Exception::ParseError, "ambient_support attribute need Tizen 2.3.1");
            }
            if (m_ambient == L"enable") {
                m_data.ambient = L"true";
            } else if (m_ambient == L"disable") {
                m_data.ambient = L"false";
            } else {
                ThrowMsg(Exception::ParseError, "Wrong ambient_support value. enable or disable");
            }
        }
    }
          ...
          private:
            DPL::OptionalInt ConvertToInt(const DPL::String& intAsString)
            {
                ...
                if ((errno == ERANGE && (intAsString_i == LONG_MAX || intAsString_i == LONG_MIN))
                        || intAsString_i > INT_MAX || intAsString_i < INT_MIN //Intended code for INT_MAX/MIN
                        || *endptr != '\0') {
                    return DPL::OptionalInt();
                }
...
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_key, MAX_NAME_KEY_LENGTH, true);
        NormalizeString(m_value, MAX_NAME_KEY_VALUE_LENGTH, true);
#else
...
class ImeParser : public ElementParser
{
  public:
    struct UuidParser : public ElementParser
    {
      public:
        ...
        virtual void Verify()
        {
            ...
            uuid_t uu;
            if (uuid_parse(DPL::ToUTF8String(m_uuid).c_str(), uu)){
                ThrowMsg(Exception::ParseError, "invalid uuid");
            }
    ...
    virtual void Verify()
    {
        ...
        if (m_uuidTagCount > 1) {
            ThrowMsg(Exception::ParseError, "As per Tizen 2.3 wrt core spec, occurence of uuid must be 1");
        }

        if (m_languagesTagCount > 1) {
            ThrowMsg(Exception::ParseError, "As per Tizen 2.3 wrt core spec, occurence of languages must be 1");
        }
  ...
  private:
    ConfigParserData& m_data;
    ConfigParserData::ImeAppInfo m_imeAppInfo;
    unsigned char m_uuidTagCount;
    unsigned char m_languagesTagCount;
};
#endif
...
{
    if (m_data.imeAppInfoList.size() > 1) {
        ThrowMsg(Exception::ParseError, "ime element must not be more than one");
        return;
    } else if (m_data.imeAppInfoList.size() == 1) {
        if (m_data.categoryList.end() == m_data.categoryList.find(L"http://tizen.org/category/ime")) {
            ThrowMsg(Exception::ParseError,\
    ...
    if (!!m_widgetId) {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_widgetId, MAX_ATTR_ELEMENT_LENGTH, true);
#else
    ...
    if (!!m_minVersion) {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_minVersion, MAX_ATTR_ELEMENT_LENGTH, true);
#else
    ...
    if (!!m_defaultlocale) {
#if ENABLE(ELEMENT_ATTR_MAX_LENGTH)
        NormalizeString(m_defaultlocale, MAX_ATTR_ELEMENT_LENGTH, true);
#else
