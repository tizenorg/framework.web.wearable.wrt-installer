...
void Manifest::serialize(xmlTextWriterPtr writer)
{
    startElement(writer, "manifest");
    {
        ...
        writeAttribute(writer, "api-version", this->apiVersion,
...
{
    bool isWatchApp = false;
    FOREACH(type, this->appCategory)
    {
        if (*type == L"com.samsung.wmanager.WATCH_CLOCK" ||
            *type == L"http://tizen.org/category/wearable_clock") {
            isWatchApp = true;
        }
    }
    if (isWatchApp) {
        startElement(writer, "watch-application");
    } else {
        startElement(writer, "ui-application");
    }
    writeAttribute(writer, "appid", this->appid);
    writeAttribute(writer, "exec", this->exec);
    if (!this->ambient.empty()) {
        writeAttribute(writer, "ambient-support", this->ambient);
    }
    writeAttribute(writer, "type", this->type);
    if (!!this->multiple) {
