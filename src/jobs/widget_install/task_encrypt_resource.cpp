...
int ssmEncrypt(int isPreload, std::string pkgId, const char*
    ...
    if (!(m_context.mode.rootPath == InstallMode::RootPath::RO &&
        ...
        do
        {
            ...
            if (0 != bytesRead) {
                ...
                // handle CSC update to download update
                if (m_context.mode.installTime == InstallMode::InstallTime::CSC &&
                    !m_context.isUpdateMode) {
                    isPreload = 1;
                }

                if (0 != ssmEncrypt(isPreload, pkgId,
